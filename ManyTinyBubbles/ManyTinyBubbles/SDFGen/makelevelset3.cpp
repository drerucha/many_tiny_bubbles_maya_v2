                     ...
                     Array3f &phi, const int exact_band)
{
   ...
   for(unsigned int t = 0; t < tri.size(); ++t){
     unsigned int p, q, r; assign(tri[t], p, q, r);//p q r 是三角形的三個點
      ...
      for(int k=k0; k<=k1; ++k) for(int j=j0; j<=j1; ++j){
         ...
         if(point_in_triangle_2d(j, k, fjp, fkp, fjq, fkq, fjr, fkr, a, b, c)){
            ...
            if(i_interval<0) 
				...
				++intersection_count(i_interval,j,k); // we ignore intersections that are beyond the +x side of the grid
         }
      }
   }
...
void make_level_set_inputPoint(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
                     const Vec3f &origin, float dx, int ni, int nj, int nk,
                     Array3f &phi, const int exact_band2)
{
   //phi.resize(ni, nj, nk);
   //phi.assign((ni+nj+nk)*dx); // upper bound on distance
   Array3i closest_tri(ni, nj, nk, -1);
   ...
   for(unsigned int t = 0; t < tri.size(); ++t)
   {
     ...
     // coordinates in grid to high precision
      ...
      double fir=((double)x[r][0] - origin[0])/dx, fjr=((double)x[r][1] - origin[1])/dx, fkr=((double)x[r][2] - origin[2])/dx;
      // do distances nearby
      int i0 = clamp(int(min(fip,fiq,fir)) - exact_band2, 0, ni-1), i1 = clamp(int(max(fip,fiq,fir))+exact_band2+1, 0, ni-1);
      int j0 = clamp(int(min(fjp,fjq,fjr)) - exact_band2, 0, nj-1), j1 = clamp(int(max(fjp,fjq,fjr))+exact_band2+1, 0, nj-1);
      int k0 = clamp(int(min(fkp,fkq,fkr)) - exact_band2, 0, nk-1), k1 = clamp(int(max(fkp,fkq,fkr))+exact_band2+1, 0, nk-1);
      for(int k = k0; k <= k1; ++k) 
		for(int j = j0; j <= j1; ++j) 
			for(int i = i0; i <= i1; ++i)
			  {
